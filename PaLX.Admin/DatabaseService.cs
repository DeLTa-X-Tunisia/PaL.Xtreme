...
namespace PaLX.Admin
{
    ...
    public class DatabaseService
    {
        ...
        public void SendSystemAlert(string sender, string receiver, string message)
        {
            try
            {
                using (var conn = new NpgsqlConnection(GetConnectionString(DatabaseName)))
                {
                    conn.Open();
                    var sql = @"
                        INSERT INTO ""Messages"" (""SenderId"", ""ReceiverId"", ""Content"", ""Type"", ""Timestamp"", ""IsRead"")
                        VALUES (
                            (SELECT ""Id"" FROM ""Users"" WHERE ""Username"" = @s),
                            (SELECT ""Id"" FROM ""Users"" WHERE ""Username"" = @r),
                            @c, 'SystemAlert', NOW(), FALSE
                        )";
                    using (var cmd = new NpgsqlCommand(sql, conn))
                    {
        ...
        public int SendMessage(string sender, string receiver, string content, string type = "Text")
        {
            ...
            { 
                System.Diagnostics.Debug.WriteLine("Send Message: " + ex.Message); 
                return 0;
            }
        }
